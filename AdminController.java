...
public class AdminController {
	...
	adminService.updateAdmin(id, admin);
	 ...
	 public ResponseEntity<List<Admin>> getAdmin(@PathVariable String name) {
		 ...
		 public ResponseEntity<List<Admin>> getAdmin1(@PathVariable int price) {
			 List<Admin> adminList = adminService.getAdminByPrice(price);
			 
			 return new ResponseEntity<List<Admin>>(adminList, HttpStatus.OK);
			 
		 }
		 @GetMapping ("/priceBetween/{start}{end}")
		 public ResponseEntity<List<Admin>> getAdminPrice(@PathVariable int price) {
			 List<Admin> adminList = adminService.getAdminByPriceBetween(price);
